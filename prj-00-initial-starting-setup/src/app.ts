...
class ProjectState extends State<Project> {
  ...
  private updateListeners() {
    for (const listenerFn of this.listeners) {
      // using slice for using a copy of the array, not the original one
...
class ProjectList extends Component<HTMLDivElement, HTMLElement> implements DragTarget {
  ...
  @Autobind
  dragHoverHanlder(event: DragEvent) {
  ...
  configure() {
    this.element.addEventListener("dragover", this.dragHoverHanlder);
...
class ProjectInput extends Component<HTMLDivElement, HTMLFormElement> {
  ...
  private gatherUserInput(): [string, string, number] | void {
    ...
    const peopleValidatable: Validatabale = {
      value: +enteredPeople,
      required: true,
      min: 1,
      max: 5,
    ...
    // validation, it will be more scalable soon
    if (
